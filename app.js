...
function statFor(id){const k=String(id);return state.stats[k]||(state.stats[k]={attempts:0,correct:0,wrong:0})}
...
function smartWeight(q){const st=state.stats[q.id];if(!st||!st.attempts)return 60+Math.random()*20;const rate=st.correct/st.attempts;return (1-rate)*100+Math.min(st.wrong,5)*12-Math.min(st.attempts,10)+Math.random()*8}
function filteredQuestions(){const source=document.getElementById('source').value;let list=[...QUESTIONS];if(source==='smart'){list.sort((a,b)=>smartWeight(b)-smartWeight(a));return list}if(source==='images')list=list.filter(q=>q.img);if(source==='noimages')list=list.filter(q=>!q.img);if(source==='wrong')list=list.filter(q=>(state.stats[q.id]?.wrong||0)>0);if(source==='unseen')list=list.filter(q=>(state.stats[q.id]?.attempts||0)===0);if(source==='favorites')list=list.filter(q=>state.favorites.includes(q.id));if(source==='range'){const a=+document.getElementById('rangeFrom').value||1,b=+document.getElementById('rangeTo').value||949;list=list.filter(q=>q.id>=Math.min(a,b)&&q.id<=Math.max(a,b))}if(source==='hard')list=list.filter(q=>(state.stats[q.id]?.attempts||0)>0).sort((a,b)=>{const sa=state.stats[a.id],sb=state.stats[b.id];return (sa.correct/sa.attempts)-(sb.correct/sb.attempts)||(sb.attempts-sa.attempts)});else shuffle(list);return list}
...
{id:'master',e:'👑',n:'Operator kompletny',d:'Opanuj 90% całej bazy',ok:()=>masteryPercent()>=90}
];
...
function diagnosticChecks(){
 ...
 const funcs=['startNew','render','choose','nextQuestion','finish','backToMenu','showStats','showQuestionBrowser','showAchievements','exportData','importData','startExamSimulator','startDailyChallenge','goToExamQuestion','toggleExamFlag'];
 const checks=[
  ...
  ['Spójność wersji',document.title.includes('3.2.0')&&document.querySelector('h1')?.textContent.includes('3.2.0')&&document.querySelector('.footer')?.textContent.includes('3.2.0'),'tytuł, nagłówek i stopka']
 ];
 return checks;
}
