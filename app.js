...
try{
  ...
  const planSession=raw?JSON.parse(raw):null;
  if(planSession){
    const target=Math.max(1,Number(planSession.target)||8);
    const machineId=planSession.machineId;
    if(completed&&answeredCount>=target&&machineId){
      const now=new Date();now.setMinutes(now.getMinutes()-now.getTimezoneOffset());
      const day=now.toISOString().slice(0,10),key=`udt_home_plan_v621_${machineId}`;
      let plan={};try{plan=JSON.parse(localStorage.getItem(key)||'{}')||{}}catch(_){plan={}}
      if(plan.day!==day)plan={day,theory:false,oral:false,tech:false,games:0};
      plan.theory=true;localStorage.setItem(key,JSON.stringify(plan));
    }
    sessionStorage.removeItem('udt_home_theory_session');
  }
}catch(e){console.error('Nie udało się zapisać teorii w Planie dnia',e);sessionStorage.removeItem('udt_home_theory_session')}
if(wasSimulator&&pct>=75)setTimeout(launchConfetti,120);renderReview(wrong,unansweredItems);document.getElementById('retryWrongBtn').classList.toggle('hidden',wrong.length===0);window.__lastWrong=wrong.map(x=>QUESTIONS.find(q=>q.id===x.id))}
...
function diagnosticChecks(){
 ...
 const checks=[
  ...
  ['Spójność wersji',document.title.includes('6.3.0')&&document.querySelector('h1')?.textContent.includes('6.3.0')&&document.querySelector('.footer')?.textContent.includes('6.3.0'),'tytuł, nagłówek i stopka']
 ];
 return checks;
}
