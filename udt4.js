/* UDT Trainer 5.1.0 — coach dashboard, SRS, readiness, activity calendar, settings */
const UDT4_VERSION='5.1.0';
...
function ensureUdt4UI(){
  ensureProState();
  const title=document.getElementById('appTitle');if(title)title.textContent='UDT Trainer 5.1.0';
  document.title='UDT Trainer 5.1.0 — trener operatora';
  ...
  const footer=document.querySelector('.footer');if(footer)footer.textContent='UDT Trainer 5.1.0 — trener operatora • PWA offline • inteligentne powtórki • dane lokalne';
  const diag=document.querySelector('#diagnostics .version-pill');if(diag)diag.textContent='Wersja 5.1.0';
}
